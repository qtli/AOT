...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(config.emb_dim, config.rnn_hidden_dim, num_layers=1, batch_first=True, bidirectional=True)
...
class ReduceState(nn.Module):
    def __init__(self):
        ...
        (self.reduce_h)
        self.reduce_c = nn.Linear(config.rnn_hidden_dim * 2, config.rnn_hidden_dim)
        init_linear_wt(self.reduce_c)

    def forward(self, hidden):
        '''

        :param hidden: h, c (2, bsz, hidden_dim)
        :return:
        '''
        h, c = hidden  # h, c dim = 2 x b x hidden_dim
        h_in = h.transpose(0, 1).contiguous().view(-1, config.rnn_hidden_dim * 2)
        hidden_reduced_h = F.relu(self.reduce_h(h_in))
        c_in = c.transpose(0, 1).contiguous().view(-1, config.rnn_hidden_dim * 2)
...
class Attention(nn.Module):
    def __init__(self):
        ...
        self.decode_proj = nn.Linear(config.rnn_hidden_dim * 2, config.rnn_hidden_dim * 2)
        self.v = nn.Linear(config.rnn_hidden_dim * 2, 1, bias=False)

    def forward(self, s_t_hat, encoder_outputs, encoder_feature, enc_padding_mask, coverage):
        '''

        :param s_t_hat: (B x 2*hidden_dim) final state of encoder or..  (decoder state)
        ...
        dec_fea = self.decode_proj(s_t_hat)  # B x 2*hidden_dim, 2 because decoder is birdirectional, too.
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(config.emb_dim, config.rnn_hidden_dim, num_layers=1, batch_first=True, bidirectional=False)
        init_lstm_wt(self.lstm)
    ...
    def forward(self, y_t_1_embd, s_t_1, encoder_outputs, encoder_feature, enc_padding_mask, c_t_1, coverage, step):
        if not self.training and step == 0:
            h_decoder, c_decoder = s_t_1  # final state from lstm encoder (1, bsz, hidden_dim)
            s_t_hat = torch.cat((h_decoder.view(-1, config.rnn_hidden_dim),
                                 c_decoder.view(-1, config.rnn_hidden_dim)), 1)  # B x 2*hidden_dim
        ...
        # the input to LSTM is the combination of context vector c_t and y_input, c_t(c_t_1) is initialized with a zero vector.
        ...
        output = torch.cat((lstm_out.view(-1, config.rnn_hidden_dim), c_t), 1) # B x hidden_dim * 3
        output = self.out(output)  # B x hidden_dim

        return output, s_t, c_t, attn_dist, coverage, p_gen
...
class PGNet(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        for di in range(max_tgt_len):
            ...
            logit, s_t_1, c_t_1, attn_dist, next_coverage, p_gen = self.decoder(y_t_1, s_t_1,
                                                                                encoder_outputs,
                                                                                encoder_feature,
                                                                                mask_src, c_t_1,
                                                                                coverage, di)
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        ys = torch.zeros(enc_batch.size(0)).fill_(config.SOS_idx).long().to(config.device)  # when testing, we set bsz into 1
        decoded_words = []
