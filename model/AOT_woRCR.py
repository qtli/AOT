...
class Decoder(nn.Module):
    ...
    def kl_divergence(self, p_logit, q_logit):
        ...
        :return:
        '''
        p = F.softmax(p_logit, dim=-1)  # target dist
        _kl = torch.sum(p * (F.log_softmax(p_logit) - torch.log(q_logit + 1e-24)), 1)
        return torch.mean(_kl)

    def forward(self, inputs, inputs_rank=None, encoder_output=None, aln_rank=None, aln_mask_rank=None, mask=None):
        ...
        if(self.universal):
            ...
            if config.aln_feature:
                ...
                encoder_output += self.align_proj(self.alignment_feature(aln_rank))
            ...
            if config.aln_loss:
                ...
                aln_loss = self.kl_divergence(tgt_attn, pred_attn)
            else:
                aln_loss = 0
...
class woRCR(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        tid_batch = batch['tags_idx_batch']  # tag indexes sequence (bsz, tgt_len)
        ...
        loss = loss + cla_loss
        if torch.isnan(loss).sum().item() != 0 or torch.isinf(loss).sum().item() != 0:
            print("check")
            pdb.set_trace()
        if train:
            loss.backward()
        ...
        if config.label_smoothing:
            ...
            return loss.item(), math.exp(min(loss.item(), 100)), cla_loss, sa_acc
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        for i in range(max_dec_step+1):
            if config.project:
                out, attn_dist, aln_loss = self.decoder(inputs=self.embedding_proj_in(self.embedding(ys)),
                                                        encoder_output=self.embedding_proj_in(encoder_outputs),
                                                        mask=(mask_src,mask_trg))
            else:
                out, attn_dist, aln_loss = self.decoder(inputs=self.embedding(ys),
                                                        encoder_output=encoder_outputs,
                                                        mask=(mask_src,mask_trg))
            prob = self.generator(out, attn_dist,
