...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
        ...
        if config.model in ['AOTNet', 'woSSE', 'AOTNet_woalnloss']:
            self.multi_head_attention_enc_dec = MultiHeadAttention_aln(hidden_size, total_key_depth, total_value_depth,
                                                                       hidden_size, num_heads, None, attention_dropout)
        else:
            self.multi_head_attention_enc_dec = MultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
                                                                   hidden_size, num_heads, None, attention_dropout)
        ...
        self.embed_rank = nn.Embedding(num_embeddings=50, embedding_dim=config.emb_dim,padding_idx=config.PAD_idx)
        self.rank_linear = nn.Linear(config.emb_dim, config.emb_dim)

        self.dropout = nn.Dropout(layer_dropout)
        self.layer_norm_mha_dec = LayerNorm(hidden_size)
        self.layer_norm_mha_enc = LayerNorm(hidden_size)
        self.layer_norm_ffn = LayerNorm(hidden_size)
        self.align_proj = nn.Linear(config.emb_dim, config.emb_dim)
    ...
    def forward(self, inputs):
        ...
        NOTE: Inputs is a tuple consisting of decoder inputs and encoder output
        """
        x, encoder_outputs, attention_weight_prev, mask = inputs
        ...
        # add alignment feature (v2) todo
        # if config.aln_feature:
        #     x += self.align_proj(self.alignment_feature(x_rank))
        #     encoder_outputs += self.align_proj(self.alignment_feature(aln_rank))
        ...
        if config.aln_loss and torch.is_tensor(attention_weight_prev) and config.hop > 1:
            attention_weight = torch.cat((attention_weight_prev.unsqueeze(1),
                                          attention_weight.unsqueeze(1)),dim=1)
            del attention_weight_prev
...
class MultiHeadAttention_aln(nn.Module):
    """
    Multi-head attention as per https://arxiv.org/pdf/1706.03762.pdf
    Refer Figure 2
    """
    ...
    def __init__(self, input_depth, total_key_depth, total_value_depth, output_depth,
        ...
        Parameters:
            ...
            dropout: Dropout probability (Should be non-zero only during training)
        """
        super(MultiHeadAttention_aln, self).__init__()
        # Checks borrowed from
        # https://github.com/tensorflow/tensor2tensor/blob/master/tensor2tensor/layers/common_attention.py
        # if total_key_depth % num_heads != 0:
        #     raise ValueError("Key depth (%d) must be divisible by the number of "
    ...
    def forward(self, queries, keys, values, mask):
        ...
        :param queries: (bsz, tgt_len, emb_dim)
        :param keys: (bsz, tgt_len, src_len, emb_dim)
        :param values: (bsz, tgt_len, src_len, emb_dim)
        :param mask: (bsz, 1, src_len)
        :return:
        '''
        bsz, tgt_len, src_len, emb_dim = keys.size()
        queries = queries.view(bsz * tgt_len, emb_dim).unsqueeze(1)  # (bsz * tgt_len, 1, emb_dim)
        keys = keys.view(bsz * tgt_len, src_len, emb_dim)
        values = values.view(bsz * tgt_len, src_len, emb_dim)
        ...
        if mask is not None:
            mask = mask.unsqueeze(1).expand(-1, tgt_len, -1, -1).reshape(bsz * tgt_len, 1, src_len)
            mask = mask.unsqueeze(1)  # [B, 1, 1, T_values]
            logits = logits.masked_fill(mask, -1e18)

        ## attention weights
        attetion_weights = logits.sum(dim=1) / self.num_heads  # (bsz*tgt_len, 1, src_len)
        attetion_weights = attetion_weights.squeeze(1).view(bsz, tgt_len, src_len)
        ...
        # return outputs, attetion_weights
        outputs = outputs.squeeze(1).view(bsz, tgt_len, emb_dim)
        return outputs, torch.softmax(attetion_weights, dim=-1)
...
def gen_embeddings(vocab):
    ...
    if config.emb_file is not None:
        ...
        emb_dict = pickle.load(open(config.emb_file, 'rb'))
        pre_trained = len(emb_dict)

        for w in emb_dict:
            if len(emb_dict[w]) == config.emb_dim:
...
class LabelSmoothing(nn.Module):
    ...
    def label_smotthing_loss(self, y_pred, y_ext, y_mask, avg=True):
        '''
        :param y_pred: (bsz, tgt_len, dict_size)
        :param y_ext: (bsz, tgt_len)  with end id and oov id
        :param y_mask: (bsz, tgt_len)
        :param avg:
        :return:
        '''
        bsz, tgt_len = y_ext.size()
        y_pred = torch.log(y_pred.clamp(min=1e-8))  # log_softmax
        # y_ext has been in transpose, so add contiguous() before use view().
        loss = self.smoothing(y_pred.view(bsz*tgt_len, -1), y_ext.contiguous().view(bsz*tgt_len, -1))
        if avg:
