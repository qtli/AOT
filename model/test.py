    ...
    def forward(self, inputs, inputs_rank=None, encoder_output=None, aln_rank=None, aln_mask_rank=None, mask=None):
        ...
        if(self.universal):
            ...
            if config.aln_feature:
                ...
                encoder_output_ = encoder_output + self.align_proj(self.alignment_feature(aln_rank))
                del encoder_output
            else:
                encoder_output_ = encoder_output
            
            # Run decoder
            y, _, _, _, attn_dist, _ = self.dec((x, inputs_rank, encoder_output_, aln_rank, [], (mask_src, dec_mask)))
            ...
            if config.aln_loss:
                attn_dist_ = torch.mean(attn_dist, dim=1)  # take average on layers
                bsz, tgt_len, src_len = attn_dist_.size()
                ...
                aln_loss = self.kl_divergence(aln_mask_rank.contiguous().view(bsz * tgt_len, src_len),
                                              attn_dist_.contiguous().view(bsz * tgt_len, src_len))
                del attn_dist_
...
class Test(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        for t in range(max_tgt_len):
            pre_logit, attn_dist, _ = self.decoder(inputs=self.embedding(ys),
                                                inputs_rank=ys_rank,
            ...
            if config.pointer_gen:
                loss += self.criterion(logit.contiguous().view(-1, logit.size(-1)), dec_ext_batch[:, :(t+1)].contiguous().view(-1))
            else:
                loss += self.criterion(logit.contiguous().view(-1, logit.size(-1)), dec_batch[:, :(t+1)].contiguous().view(-1))

            if config.label_smoothing:
                loss_ppl += self.criterion_ppl(logit.contiguous().view(-1, logit.size(-1)),
                                              dec_ext_batch[:, :(t+1)].contiguous().view(-1) if config.pointer_gen else dec_batch.contiguous().view(-1))
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        for i in range(max_dec_step+1):
            if config.project:
                out, attn_dist = self.decoder(inputs=self.embedding_proj_in(self.embedding(ys)),
                                              encoder_output=self.embedding_proj_in(encoder_outputs),
                                              mask=(mask_src,mask_trg))
            else:
                out, attn_dist = self.decoder(inputs=self.embedding(ys),
                                              encoder_output=encoder_outputs,
                                              mask=(mask_src,mask_trg))
