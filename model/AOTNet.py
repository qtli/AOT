'''
1. calculate a salience score for each review by a Sentence-level Salience Estimation phrase.  -> cross-entropy loss
2. this reviews are feed into transformer encoder & decoder
3. after usual encoder(feed a long sequence), then we split long encoding sequence into many reviews by their true length for each sample.
4. for each sample, reviews are feed into on-line kmeans. by clustering, we acquire each cluster's rank
5. to feed into as many as possible reviews, we concate all reviews into a long sequence without padding in the middle.
'''
import torch
...
class AOT(nn.Module):
    def __init__(self, vocab, model_file_path=None, is_eval=False, load_optim=False):
        ...
        self.sse = SSE(vocab, config.emb_dim, config.dropout, config.rnn_hidden_dim)
        self.rcr = RCR()
    ...
    def train_one_batch_slow(self, batch, iter, train=True):
        ...
        src_batch = batch['reviews_batch']  # reviews sequence (bsz, r_num, r_len)
        src_mask = batch['reviews_mask']  # indicate which review is fake(for padding).  (bsz, r_num)
        ...
        cla_loss, sa_scores, sa_acc = self.sse.salience_estimate(src_batch, src_mask, src_length, src_labels)  # sa_scores: (bsz, r_num)
        ...
        for idx in range(bsz):  # Custering (by k-means) and Ranking
            ...
            for r_idx in range(len(item_length)):
                if r_idx == len(item_length) - 1:
                    ...
                    break
                r = self.rcr.hierarchical_pooling(reviews[r_idx].unsqueeze(0)).squeeze(0).detach().cpu().numpy() * sa_scores[idx,r_idx]
                r_vectors.append(r)
        ...
        for t in range(max_tgt_len):
            ...
            if iter >= 13000:
                ...
                loss += aln_loss_cur
            logit = self.generator(pre_logit, attn_dist.unsqueeze(1), enc_batch_extend_vocab if config.pointer_gen else None, extra_zeros)
        ...
        if config.label_smoothing:
            ...
            if torch.isnan(loss_ppl).sum().item() != 0 or torch.isinf(loss_ppl).sum().item() != 0:
                print("check")
                pdb.set_trace()
            return loss_ppl.item(), math.exp(min(loss_ppl.item(), 100)), cla_loss.item(), sa_acc
        else:
            return loss.item(), math.exp(min(loss.item(), 100)), cla_loss.item(), sa_acc
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        for i in range(max_dec_step + 1):
            ...
            if config.attn_analysis:
                pred_attn_dist = torch.cat((pred_attn_dist, attn_dist.unsqueeze(1)),dim=1)
        ...
        if config.attn_analysis:
            ...
            tgt_attn = aln_mask_rank.view(bsz * tgt_len, src_len)
            good_attn_sum = torch.masked_select(pred_attn, tgt_attn.bool()).sum()  # pred_attn: bsz * tgt_len, src_len
            bad_attn_sum = torch.masked_select(pred_attn, ~tgt_attn.bool()).sum()
            bad_num = ~tgt_attn.bool()
            ratio = bad_num.sum() / tgt_attn.bool().sum()
            bad_attn_sum /= ratio


            good_attn = good_attn_sum[0]  # last step (because this's already been the whole sentence length.).
